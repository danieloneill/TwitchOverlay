...
AuthWindow {
    ...
    {
        show();
        url = 'https://id.twitch.tv/oauth2/authorize?client_id='+settings.clientid+'&redirect_uri='+twitchApi.m_authurl+'&response_type=code&scope=chat:read';
    }
    ...
    onLoadingChanged: function (loadRequest) {
        ...
        if( url.substr(0,twitchApi.m_authurl.length) === twitchApi.m_authurl )
        {
            ...
            else if( loadRequest.status === false )
            {
                ...
                const querystr = url.substr(twitchApi.m_authurl.length + 1);
                ...
                const params = 'client_id='+encodeURIComponent(settings.clientid)
                            ...
                            +encodeURIComponent(twitchApi.m_authurl);

                console.log("Posting: "+nurl+" => "+params);
                twitchApi.httpPostRequester(nurl, function(contents) {
